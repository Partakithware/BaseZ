...
void decode_file(const char* input_path, const char* output_path) {
    ...
    size_t read_chars;  // <-- you need this variable to hold output length

   while (bytes_written < filesize) {
    int count = 0;
    while (count < ENCODED_BLOCK_SIZE) {
        int c = fgetc(fin);
        if (c == EOF) {
            fprintf(stderr, "Premature EOF while reading encoded data\n");
            goto cleanup;
        }
        // Skip whitespace/newlines
        if (c == '\n' || c == '\r' || c == ' ' || c == '\t')
            continue;
        encoded[count++] = (char)c;
    }
